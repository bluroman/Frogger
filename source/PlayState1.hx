...
class PlayState1 extends FlxState
{
	...
	override public function create():Void 
	{
		...
		coins = new FlxGroup();
		level = new TiledLevel1("assets/tiled/frogger2.tmx", this);
	...
	override public function update(elapsed:Float):Void 
	{
		...
		if (FlxG.keys.anyPressed([W, UP]) )//&& player.isTouching(FlxObject.FLOOR))
		{
			//player.velocity.y = -player.maxVelocity.y / 2;
            player.y -= 3;
		}
        if (FlxG.keys.anyPressed([S, DOWN]) )//&& player.isTouching(FlxObject.FLOOR))
        {
            //player.velocity.y = -player.maxVelocity.y / 2;
            player.y += 3;
        }
